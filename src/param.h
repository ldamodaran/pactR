...
public:
	Parameters(const std::string& paramFile = "in.param");  // imports parameters from file
	...
	bool prune_to_label;
	vector<std::string> prune_to_label_values;	// label

	bool prune_to_tips;
	vector<std::string> prune_to_tips_values;	// list of tips

	bool remove_tips;
	vector<std::string> remove_tips_values;		// list of tips
	...
	bool ordering;
	vector<std::string> ordering_values;

	bool pairs_diversity;
	vector<double> pairs_diversity_values;	// time diff

private:
	void importLine(std::string);		// reads a string and attempts to extract parameters from it
