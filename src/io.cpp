...
IO::IO(const std::string& treesFile, const std::string& paramFile, const std::string& outPrefix)
    : param(paramFile), inputFile(treesFile), outputPrefix(outPrefix)
{
	// PARAMETER INPUT
	param.print();

	// TREE INPUT
	Rcpp::Rcout << "Reading trees from " << inputFile << std::endl;
	...
	if (inStream.is_open()) {
		...
		throw runtime_error("tree file not found: " + inputFile);
	}
	...
	if (param.printtree()) {
		...
		outStream.open(outputFile.c_str(), std::ios::out);
		outStream.close();
	}
...
void IO::treeManip() {
	...
	if (param.manip()) {
		...
		Rcpp::Rcout << "Performing tree manipulation operations" << std::endl;
...
void IO::printTree() {
	...
	if (param.print_tree || param.print_circular_tree) {
		...
		Rcpp::Rcout << "Printing tree with highest posterior probability to " << outputFile << std::endl;
	...
	if (param.print_all_trees) {
		...
		Rcpp::Rcout << "Printing trees to trees/ directory" << std::endl;
...
void IO::printStatistics() {
	...
	if (param.summary()) {
		...
		ofstream outStream;
		outStream.open(outputFile.c_str(), std::ios::app);

		outStream << "statistic\tlower\tmean\tupper" << std::endl;
		...
		if (param.summary_tmrca) {
			Rcpp::Rcout << "Printing TMRCA summary to " << outputFile << std::endl;
			Series s;
			for (int i = 0; i < (int)treelist.size(); i++) {
				CoalescentTree ct = treelist[i];
			...
			outStream << "tmrca" << "\t";
			outStream << s.quantile(0.025) << "\t" << s.mean() << "\t" << s.quantile(0.975) << std::endl;
		}

		// LENGTH
		if (param.summary_length) {
			Rcpp::Rcout << "Printing length summary to " << outputFile << std::endl;
			Series s;
			for (int i = 0; i < (int)treelist.size(); i++) {
				CoalescentTree ct = treelist[i];
		...
		if (param.summary_root_proportions) {
			Rcpp::Rcout << "Printing root proportion summary to " << outputFile << std::endl;
		...
		if (param.summary_proportions) {
			Rcpp::Rcout << "Printing trunk proportion summary to " << outputFile << std::endl;
			for (is = lset.begin(); is != lset.end(); ++is) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
					CoalescentTree ct = treelist[i];
		...
		if (param.summary_coal_rates) {
			Rcpp::Rcout << "Printing coalescent summary to " << outputFile << std::endl;
			if (lset.size()>1) {
				for (is = lset.begin(); is != lset.end(); ++is) {
					Series s;
					for (int i = 0; i < (int)treelist.size(); i++) {
		...
		if (param.summary_mig_rates) {
			Rcpp::Rcout << "Printing migration summary to " << outputFile << std::endl;
		...
		if (param.summary_sub_rates) {
			Rcpp::Rcout << "Printing substitution rate summary to " << outputFile << std::endl;
			Series s;
			for (int i = 0; i < (int)treelist.size(); i++) {
				CoalescentTree ct = treelist[i];
		...
		if (param.summary_diversity) {
			Rcpp::Rcout << "Printing diversity summary to " << outputFile << std::endl;
			if (lset.size()>1) {
				for (is = lset.begin(); is != lset.end(); ++is) {
					Series s;
					for (int i = 0; i < (int)treelist.size(); i++) {
		...
		if (param.summary_fst) {
			Rcpp::Rcout << "Printing FST summary to " << outputFile << std::endl;
			Series s;
			for (int i = 0; i < (int)treelist.size(); i++) {
				CoalescentTree ct = treelist[i];
				double n = ct.getFst();
		...
		if (param.summary_tajima_d) {
			Rcpp::Rcout << "Printing Tajima's D summary to " << outputFile << std::endl;
			Series s;
			for (int i = 0; i < (int)treelist.size(); i++) {
				CoalescentTree ct = treelist[i];
		...
		if (param.summary_persistence) {
			Rcpp::Rcout << "Printing persistence summary to " << outputFile << std::endl;
			...
			outStream << lower << "\t" << avg << "\t" << upper << std::endl;
		...
		if (param.summary_diffusion_coefficient) {
			...
			Rcpp::Rcout << "Printing coefficients of diffusion to " << outputFile << std::endl;
			Series s;
			for (int i = 0; i < (int)treelist.size(); i++) {
				CoalescentTree ct = treelist[i];
			...
			outStream << s.quantile(lowerQuantile) << "\t" << s.mean() << "\t" << s.quantile(upperQuantile) << std::endl;
		...
		if (param.summary_drift_rate) {
			...
			Rcpp::Rcout << "Printing drift rate to " << outputFile << std::endl;
			Series s;
			for (int i = 0; i < (int)treelist.size(); i++) {
				CoalescentTree ct = treelist[i];
...
void IO::printSkylines() {
	...
	if (param.skyline()) {
		...
		outStream << "statistic\ttime\tlower\tmean\tupper" << std::endl;
		...
		if (param.skyline_tmrca) {
			Rcpp::Rcout << "Printing TMRCA skyline to " << outputFile << std::endl;
		...
		if (param.skyline_length) {
			Rcpp::Rcout << "Printing length skyline to " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
					CoalescentTree ct = treelist[i];
		...
		if (param.skyline_proportions) {
			Rcpp::Rcout << "Printing proportions skyline to " << outputFile << std::endl;
		...
		if (param.skyline_coal_rates) {
			Rcpp::Rcout << "Printing coalescent skyline to " << outputFile << std::endl;
		...
		if (param.skyline_mig_rates) {
			Rcpp::Rcout << "Printing migration skyline to " << outputFile << std::endl;
		...
		if (param.skyline_pro_history_from_tips) {
			Rcpp::Rcout << "Printing proportion history skyline to " << outputFile << std::endl;
			for (is = lset.begin(); is != lset.end(); ++is) {
				for (js = lset.begin(); js != lset.end(); ++js) {
		...
		if (param.skyline_diversity) {
			Rcpp::Rcout << "Printing diversity skyline to " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
					CoalescentTree ct = treelist[i];
		...
		if (param.skyline_fst) {
			Rcpp::Rcout << "Printing FST skyline to " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
					CoalescentTree ct = treelist[i];
		...
		if (param.skyline_tajima_d) {
			Rcpp::Rcout << "Printing Tajima D skyline to " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
					CoalescentTree ct = treelist[i];
		...
		if (param.skyline_timetofix) {
			Rcpp::Rcout << "Printing fixation time skyline to " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
					CoalescentTree ct = treelist[i];
		...
		if (param.skyline_xmean) {
			Rcpp::Rcout << "Printing X mean skyline to " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
					CoalescentTree ct = treelist[i];
				...
				outStream << "xmean" << "\t";
				outStream << t << "\t";
				outStream << s.quantile(0.25) << "\t" << s.mean() << "\t" << s.quantile(0.75) << std::endl;
			}
		}

		// Y LOCATION
		if (param.skyline_ymean) {
			Rcpp::Rcout << "Printing Y mean skyline to " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
					CoalescentTree ct = treelist[i];
		...
		if (param.skyline_xdrift) {
			Rcpp::Rcout << "Printing X drift skyline to " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
		...
		if (param.skyline_ratemean) {
			Rcpp::Rcout << "Printing rate mean skyline to " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
					CoalescentTree ct = treelist[i];
		...
		if (param.skyline_xtrunkdiff) {
			Rcpp::Rcout << "Printing X trunk difference to " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
					CoalescentTree ct = treelist[i];
		...
		if (param.skyline_locsample) {
			Rcpp::Rcout << "Printing loc sample skyline to " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
		...
		if (param.skyline_locgrid) {
			Rcpp::Rcout << "Printing loc grid skyline to " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				...
				for (int i = 0; i < (int)treelist.size(); i++) {
					...
					vector<double> xlocs = ct.getTipsX();
					vector<double> ylocs = ct.getTipsY();
					for (int i = 0; i < (int)xlocs.size(); i++) {
						vector<double> v;
						v.push_back(xlocs[i]);
						v.push_back(ylocs[i]);
						locset.insert(v);
				...
				double gstep = 0.25;
				for (double x = -2.0; x <= 50.0; x += gstep) {
					for (double y = -6.0; y <= 6.0; y += gstep) {

						int count = 0;
						for (multiset< vector<double> >::iterator is = locset.begin(); is != locset.end(); is++ ) {
							double thisx = (*is).at(0);
							double thisy = (*is).at(1);
							if (thisx < x + 0.5*gstep && thisx > x - 0.5*gstep && thisy < y + 0.5*gstep && thisy > y - 0.5*gstep) {
								count++;
							}
						}
						outStream << "\t" << count;
		...
		if (param.skyline_drift_rate_from_tips) {
			Rcpp::Rcout << "Printing skyline of 1D drift rate from tips " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
					CoalescentTree ct = treelist[i];
					double n = ct.get1DRateFromTips(t, step);
					s.insert(n);
				}
		...
		if (param.skyline_drift_rate_from_tips) {
			Rcpp::Rcout << "Printing skyline of 2D drift rate from tips " << outputFile << std::endl;
			for (double t = start; t + step <= stop; t += step) {
				Series s;
				for (int i = 0; i < (int)treelist.size(); i++) {
					CoalescentTree ct = treelist[i];
					double n = ct.get2DRateFromTips(t, step);
					s.insert(n);
				}
...
void IO::printTips() {
	...
	if (param.tips()) {
		...
		outStream << "statistic\tname\tlabel\ttime\tlower\tmean\tupper" << std::endl;

		vector<string> tipNames = treelist[0].getTipNames();

		// TIME TO TRUNK
		if (param.tips_time_to_trunk) {
			Rcpp::Rcout << "Printing time to trunk for tips to " << outputFile << std::endl;
			for (int n = 0; n < (int)tipNames.size(); n++) {
		...
		if (param.x_loc_history) {
			...
			Rcpp::Rcout << "Printing x loc history for tips to " << outputFile << std::endl;
		...
		if (param.y_loc_history) {
			...
			Rcpp::Rcout << "Printing y loc history for tips to " << outputFile << std::endl;
...
void IO::printPairs() {
	...
	if (param.pairs()) {
		...
		outStream << "statistic\tnameA\tnameB\tlower\tmean\tupper" << std::endl;
		...
		if (param.pairs_diversity) {
			...
			Rcpp::Rcout << "Printing pairwise diversity to " << outputFile << std::endl;
			for (int nA = 0; nA < (int)tipNames.size(); nA++) {
				for (int nB = nA + 1; nB < (int)tipNames.size(); nB++) {
...
int IO::getBestTree() {
	...
	if (problist.size() == treelist.size()) {
		...
		index = 0;
		for (int i = 1; i < (int)problist.size(); i++) {
			if (problist[i] > ll) {
				ll = problist[i];
				index = i;
			}
